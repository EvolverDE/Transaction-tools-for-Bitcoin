...
Public Class ClsBitcoinTransaction
    ...
    Sub New(ByVal Inputs As List(Of ClsUnspentOutput), ByVal Outputs As List(Of ClsOutput), ByVal Addresses As List(Of S_Address))

        C_Inputs = Inputs
        C_Outputs = Outputs
        C_ScriptAddresses = Addresses
    ...
    Public Sub FinalizingOutputs(ByVal ChangeAddress As String)
    ...
    Private Sub FilterAddresses(ByVal Addresses As List(Of S_Address))
        ...
        For i As Integer = 0 To C_Inputs.Count - 1
            ...
            If T_Input.OutputType = AbsClsOutputs.E_Type.ChainSwapHashWithLockTime Then
                ...
                Dim T_SenderAddress As String = RIPE160ToAddress(GetXFromScript(T_Input.Script, ClsScriptEntry.E_OP_Code.RIPE160Sender), BitcoinAddressPrefix)
                Dim T_RecipientAddress As String = RIPE160ToAddress(GetXFromScript(T_Input.Script, ClsScriptEntry.E_OP_Code.RIPE160Recipient), BitcoinAddressPrefix)
        ...
        For i As Integer = 0 To C_Inputs.Count - 1
            ...
            For j As Integer = 0 To C_Inputs.Count - 1
                ...
                If T_Unsignedinput.OutputType = AbsClsOutputs.E_Type.Pay2ScriptHash And Not UseStandardSequence Then
                    ...
                    Dim T_timeout As String = GetXFromScript(T_Unsignedinput.Script, ClsScriptEntry.E_OP_Code.LockTime)
    ...
    Private Function CreateSignedTransaction() As String
        ...
        For j As Integer = 0 To C_Inputs.Count - 1
            ...
            If result1 And Not UseStandardSequence Then
                ...
                Dim T_timeout As String = GetXFromScript(T_Input.Script, ClsScriptEntry.E_OP_Code.LockTime)
    ...
    Public Function IsTimeOut() As Boolean
        ...
        For i As Integer = 0 To C_Inputs.Count - 1
            ...
            If T_BTCTXInput.OutputType = AbsClsOutputs.E_Type.Pay2ScriptHash Then
                ...
                Dim LockTime As String = GetXFromScript(T_BTCTXInput.Script, ClsScriptEntry.E_OP_Code.LockTime)
                LockTime = ChangeHEXStrEndian(LockTime) 'to big endian

                Dim RIPE160Sender As String = GetXFromScript(T_BTCTXInput.Script, ClsScriptEntry.E_OP_Code.RIPE160Sender)
                Dim AddressSender As String = RIPE160ToAddress(RIPE160Sender, BitcoinAddressPrefix)

                Dim RIPE160Recipient As String = GetXFromScript(T_BTCTXInput.Script, ClsScriptEntry.E_OP_Code.RIPE160Recipient)
                Dim AddressRecipient As String = RIPE160ToAddress(RIPE160Recipient, BitcoinAddressPrefix)
